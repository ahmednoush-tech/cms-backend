...
import type { CreateBillFormValues } from './CreateBillPage';

interface BillItemsEditorProps {
  control: Control<CreateBillFormValues>;
  register: UseFormRegister<CreateBillFormValues>;
  errors: FieldErrors<CreateBillFormValues>;
}

export function BillItemsEditor({ control, register, errors }: BillItemsEditorProps) {
  const { t } = useTranslation(['finance', 'common']);
  const { fields, append, remove } = useFieldArray({ control, name: 'items' });
  // Active items only — a deactivated item shouldn't be selectable for a NEW purchase line,
  ...
  return (
    <div>
      ...
      <button
        type="button"
        onClick={() => append({ description: '', quantity: '1', unitPrice: '0', discount: '0', tax: '0', inventoryItemId: '' })}
