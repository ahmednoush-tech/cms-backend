...
import type { CreatePurchaseOrderFormValues } from './CreatePurchaseOrderPage';

interface PurchaseOrderItemsEditorProps {
  control: Control<CreatePurchaseOrderFormValues>;
  register: UseFormRegister<CreatePurchaseOrderFormValues>;
  errors: FieldErrors<CreatePurchaseOrderFormValues>;
}

export function PurchaseOrderItemsEditor({ control, register, errors }: PurchaseOrderItemsEditorProps) {
      ...
      <table className="w-full text-start text-sm">
        <thead className="text-xs uppercase text-ink-muted">
          <tr>
            ...
            <th className="pb-2 text-start">{t('purchaseOrders:fields.stockItem')}</th>
            <th className="pb-2" />
          </tr>
        </thead>
